...
struct sring_gso_tx_context {
    ...
    struct sring_gso_tx_desc desc[0];
};

struct sring_gso_rx_desc {
    uint64_t cookie;
    uint64_t paddr;
    uint16_t len;
    uint16_t flags;
...
struct sring_gso_rx_context {
    ...
    struct sring_gso_rx_desc desc[0];
};
